...
### Conflict Detection
Run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" sync <change-dir>`. If conflicts are detected (same requirement modified by multiple changes), present the conflict list to the user for resolution order.
...
3. Record that merging is complete so the `executing → closing` guard allows closure:
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> spec_merged true
   ```
