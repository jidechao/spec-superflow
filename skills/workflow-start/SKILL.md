...
After confirmation:
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_0_decisions "<summary>"
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_0_result confirmed
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_0_confirmed true
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_0_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
If workflow is `auto`/`null`/unset: run `node "${CLAUDE_PLUGIN_ROOT}/scripts/infer-workflow.mjs" <change-dir>`. Inference: **hotfix** (≤2 tasks, ≤2 files, no schema/API/new modules), **tweak** (≤4 tasks, config/doc only), **full** (anything larger). Persist with `bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <dir> workflow <mode>`.
...
Post-transition: 💡 `bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" inject <change-dir>` to update phase-guard artifacts.
