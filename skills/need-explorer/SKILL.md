...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_1_result "confirmed: <one-line summary>"
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_1_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
