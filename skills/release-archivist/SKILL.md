...
- Run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" audit <change-dir>` — include `decision-point-audit.md` in archive

### DP-6 (Verification Outcome)
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_6_result "<pass|conditional|fail>: <summary>"
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_6_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> test_result pass
```

### DP-7 (Archive Confirmation)
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_7_result "confirmed: <archive summary>"
bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_7_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
Run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state transition <change-dir> closing`. If delta specs exist, route to `spec-merger`.
...
- **Missing files**: If audit can't generate, run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" audit` manually
