...
3+ failed fixes = architectural problem. Each fix revealing new problems elsewhere = wrong architecture. Record: `bash "${CLAUDE_PLUGIN_ROOT}/scripts/ssf" state set <change-dir> dp_5_result <decision>`. Discuss with user before attempting more fixes.
